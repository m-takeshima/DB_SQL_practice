...
WHERE
    (
        indep_year IS NOT NULL AND indep_year < 1990
    )
...
SELECT
    ce.name,
    (
        SELECT c.name FROM countries c WHERE c.code = ce.country_code
    ) 
    AS 国名
FROM
    celebrities ce
